...
public class DiscoveryController {
    ...
    @Autowired
    private MatchPrefrenceService matchPrefrenceService;
...
//            @RequestParam(required = false) String planId,
        ...
        matchPrefrenceService.getByUserId(userId).map(matchPreference -> {
            filterMap.put("userDetails.gender",matchPreference.getInterestedIn());
            filterMap.put("minAge",matchPreference.getMinAge());
            filterMap.put("maxAge",matchPreference.getMaxAge());
            filterMap.put("distanceInKm", matchPreference.getMaxDistance());
            return null;
        });
       ...
       userService.findByUserId(userId).map(users -> {
        ...
        if (Objects.nonNull(skillRating)) filterMap.put("userDetails.skillRating", skillRating);
        ...
        if (Objects.nonNull(lat)) filterMap.put("latitude", lat);
        if (Objects.nonNull(lon)) filterMap.put("longitude", lon);
        if (Objects.nonNull(distanceInKm)) filterMap.put("distanceInKm", distanceInKm);

        List<String> userWhichAreSwipedAlready = new ArrayList<>();
        swipeService.getSwipesByUser(userId)
                .doOnNext(user -> userWhichAreSwipedAlready.add(user.getKey().getSwipeeId())).subscribe();

        return userService.getFilteredList(filterMap, page, size, sortBy, sortOrder)
                .filter(user -> !userWhichAreSwipedAlready.contains(user.getUid())).collectList()
                .flatMap(users -> {
